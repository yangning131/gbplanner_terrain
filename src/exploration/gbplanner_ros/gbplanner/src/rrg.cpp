...
Rrg::Rrg(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
  ...
  free_cloud_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("freespace_pointcloud", 10);
       runtime_pub =  nh_.advertise<std_msgs::Float32>("/runtime", 5);
  ...
  fit_plane_arg.ratio_min_ = 0.25;
  fit_plane_arg.ratio_max_ = 0.4;
  fit_plane_arg.conv_thre_ = 0.1152;
...
void Rrg::clear() {}
    ...
    void Rrg::cloudHandler(const sensor_msgs::PointCloud2::ConstPtr& laserCloudMsg)
    {
        ...
        obs = DecompROS::cloud_to_2vec(cloud);


        receive = true;
    }

     void Rrg::cloudHandler2(const sensor_msgs::PointCloud2::ConstPtr& laserCloudMsg)
    {
        ...
        for (const auto& pt : cloud)
        {
          ...
          world->setObs(obstacle);//x ,y ,z 三个index
        }
    ...
    //       //terrain
    //       //1
    //       // pcl::PointCloud<pcl::PointXYZ> cloud_te;
          
    //       // pcl::fromROSMsg(cloud3, cloud_te);

    //       // world->initGridMap(cloud_te);
    //       // for (const auto& pt : cloud_te)
    //       // {
    //       //   Eigen::Vector3d obstacle(pt.x, pt.y, pt.z);
    //       //   world->setObs(obstacle);//x ,y ,z 三个index    有点云置为false
    //       // }
    //       //  visWorld(world, &grid_map_vis_pub);
    //       //2
    //       pcl::PointCloud<pcl::PointXYZ>::Ptr laserCloudIn(new pcl::PointCloud<pcl::PointXYZ>());
          
    //       pcl::fromROSMsg(cloud3, *laserCloudIn);
    //       cloudQueue.push_back(*laserCloudIn);
    //       timeQueue.push_back(timeScanCur);
    //       // std::cout<<"timeScanCur->timeScanCur()"<<timeScanCur<<std::endl;

    //       while (!timeQueue.empty())//大于一定范围   弹出旧的点云
    //       {
    ...
    //       world->initGridMap(*surroundMapCloudDS);
    //       // std::cout<<"surroundMapCloudDS->size()"<<surroundMapCloudDS->size()<<std::endl;
    //       for (int i=0;i<surroundMapCloudDS->size();++i)
    //       {
    //         Eigen::Vector3d obstacle(surroundMapCloudDS->points[i].x, surroundMapCloudDS->points[i].y, surroundMapCloudDS->points[i].z);
    //         world->setObs(obstacle);//x ,y ,z 三个index    有点云置为false
    //       }
    //        visWorld(world, &grid_map_vis_pub);
    //      //terrain
   ...
   bool Rrg::circle_collision(double start_x, double start_y,double goal_x,double goal_y)
   {
      StateVec check_state;
      double dir=atan2(goal_x-start_x,goal_y - start_y);
      for(float r=0;r<sqrt( (goal_x-start_x)* (goal_x-start_x)+(goal_y-start_y)*(goal_y-start_y)  ) ;r=r+0.5   )///r+5换为ros参数
       {     
          
            check_state[0] = start_x+r*sin(dir);//考虑eigen库
            check_state[1] = start_y+r*cos(dir);
            check_state[2] = 0.8;

            if(  MapManager::VoxelStatus::kFree !=
               map_manager_->getBoxStatus(
            Eigen::Vector3d(check_state[0], check_state[1], check_state[2]) +
                robot_params_.center_offset,
            robot_box_size_, true))
            return false;
       }
...
//           if(calculate_tecost)
//           {
//               std::cout<<"before_point"<<point_show<<std::endl;
//               std::cout<<"sample_point:"<<sample_point[2]<<std::endl;
  ...
  while (!found && while_thres--) {
    ...
    if (MapManager::VoxelStatus::kFree ==
      ...
      if (world->has_map_)
      {
          ...
          {
              // std::cout<<"before_point"<<point_show<<std::endl;
              // std::cout<<"sampled_point:"<<sample_point[2]<<std::endl;
      ...
      // random_sampler_.pushSample(state, true);  // for debug purpose.  //no terrain
      // found = true;                                                    ////no terrain
...
bool Rrg::sampleVertex(RandomSampler& random_sampler, StateVec& root_state,
  ...
  while (!found && while_thres--) {
    ...
    if (MapManager::VoxelStatus::kFree == map_manager_->getBoxStatus(
      ...
      Eigen::Vector3d sample_point(sampled_state[0], sampled_state[1], sampled_state[2]);
      float point_show = sampled_state[2];
      ...
      if (world->has_map_)
      {
          ...
          if(calculate_tecost)
          {
             ...
             sampled_state[2] = sample_point[2];
              ...
              if(terrain_cost<0.98) ////0.6
              {
                ...
                random_sampler_.pushSample(sampled_state, true);  // for debug purpose.
                found = true;
              }
              else {
                  stat_->num_vertices_fail++;
          ...
          {
            // std::cout<<"calculate_tecost failed:"<<std::endl;
          }
            
      }


      // random_sampler_.pushSample(sampled_state, true);  // for debug purpose.//no terrain
      // found = true;                                                           //no terrain
...
// bool Rrg::sampleVertex(StateVec& state, vec_E<Polyhedron2D>& polys) {
...
//           std::cout<<"Check if surrounding area is free."<<std::endl;
//           Vec2f new_seed(state[0], state[1]);
//           std::vector<StateVec>* Samples = random_sampler_.getSamples(true);
//           if(Samples->empty())
//           {
//                       std::cout<<"Samples->empty()"<<std::endl;

//             SeedDecomp2D decomp_util(new_seed);

//             vec_Vec2f obs_back(obs);
//               for(int i = 0;i<360;i++)
//               { 
//                 Vec2f medie;
//                 medie(0) = new_seed(0)+ 7.0*std::cos(i*3.14/180);
//                 medie(1) = new_seed(1) + 7.0*std::sin(i*3.14/180);
...
//             {
//               Vec2f seed(Samples->at(i)[0],Samples->at(i)[1]);
//               auto poly = polys[i];
//               LinearConstraint2D cs(seed, poly.hyperplanes());
//               if(cs.inside(new_seed))
//               {
//                 random_sampler_.pushSample(state, false); // for debug purpose.
//                 found = false;
//                 is_inside = true;
//                 stat_->num_vertices_fail++;
...
//             if(!is_inside)
//             {
//             std::cout<<"SeedDecomp2D decomp_util(new_seed);"<<std::endl;
...
//             {
//               Vec2f seed(Samples->at(i)[0],Samples->at(i)[1]);
//               double dist = (Samples->at(i)[0]-state[0])*(Samples->at(i)[0]-state[0]) + (Samples->at(i)[1]-state[1])*(Samples->at(i)[1]-state[1]);
...
//             if(min_dist<36)
//             {
//                 if(circle_collision(Samples->at(min_index)[0], Samples->at(min_index)[0],new_seed(0),new_seed(1) ))///collision
//                 {
...
// bool Rrg::sampleVertex(RandomSampler& random_sampler, StateVec& root_state,
//                        StateVec& sampled_state,  vec_E<Polyhedron2D>& polys) {
...
//           Vec2f new_seed(sampled_state[0], sampled_state[1]);
//           std::vector<StateVec>* Samples = random_sampler.getSamples(true);
//           if(Samples->empty())
//           {
...
//               if(cs.inside(new_seed))
//               {
//                 random_sampler.pushSample(sampled_state, false); // for debug purpose.
//                 found = false;
//                 is_inside = true;
//                 break;
//               }
...
//             {
//               Vec2f seed(Samples->at(i)[0],Samples->at(i)[1]);
//               double dist = (Samples->at(i)[0]-sampled_state[0])*(Samples->at(i)[0]-sampled_state[0]) + (Samples->at(i)[1]-sampled_state[1])*(Samples->at(i)[1]-sampled_state[1]);
//               if(dist<min_dist)
//               {
//                 min_dist = dist;
//                 min_index = i ;
...
//                             stat_->num_vertices_fail++;
//                             random_sampler_.pushSample(sampled_state, false);
//                             found = false;
...
Rrg::GraphStatus Rrg::buildGraph() {
  ...
  //走廊采样开关
  //vec_E<Ellipsoid2D> es; //corridor set
  //vec_E<Polyhedron2D> polys; //corridor set
  ...
  while ((loop_count++ < planning_params_.num_loops_max) &&
    ...
    if (planning_params_.type == PlanningModeType::kVerticalExploration) {
      if (!sampleVertex(random_sampler_vertical_, root_vertex_->state,
                        new_state))//polys  //corridor set
        continue;
    } else { 
      if (!sampleVertex(new_state)) continue;//polys  //corridor set
    }
...
void Rrg::freePointCloudtimerCallback(const ros::TimerEvent& event) {
  ...
  // pcl::PointCloud<pcl::PointXYZI>::Ptr free_cloud_body(
  //     new pcl::PointCloud<pcl::PointXYZI>);

  //   map_manager_->getsdfsurface(free_cloud_body);


  //   sensor_msgs::PointCloud2 out_cloud;
  //   pcl::toROSMsg(*free_cloud_body.get(), out_cloud);
...
void Rrg::expandGlobalGraphTimerCallback(const ros::TimerEvent& event) {
  ...
  while (time_elapsed < kGlobalGraphUpdateTimeBudget) {
    ...
    for (int i = 0; i < cluster_centroids.size(); ++i) {
      ...
      loop_count_success++;
      ExpandGraphReport rep;
      expandTreeStar(global_graph_, new_state, rep);
      if (rep.status == ExpandGraphStatus::kSuccess) {
...
ConnectStatus Rrg::findPathToConnect(
  ...
  while (!stop_sampling) {
    ...
    ExpandGraphReport rep;
    expandTreeStar(graph_manager, new_state, rep);
    if (rep.status == ExpandGraphStatus::kSuccess) {
      num_vertices += rep.num_vertices_added;
      num_edges += rep.num_edges_added;
  ...
  if (reached_target) {
    ...
    if (voxel_state == MapManager::VoxelStatus::kFree) {
      ExpandGraphReport rep;
      expandTreeStar(graph_manager, target, rep);
      if (rep.status == ExpandGraphStatus::kSuccess) {
...
std::vector<geometry_msgs::Pose> Rrg::searchHomingPath(
  ...
  if (direction_norm <= kRadiusLimit) {
    ...
    ExpandGraphReport rep;
    expandTreeStar(global_graph_, cur_state, rep);
    if (rep.status == ExpandGraphStatus::kSuccess) {
...
bool Rrg::addRefPathToGraph(const std::shared_ptr<GraphManager> graph_manager,
  ...
  if (direction_norm <= kDeltaLimit) {
    ...
    ROS_WARN("[GlobalGraph] Try to add current state to the graph.");
    ExpandGraphReport rep;
    expandTreeStar(graph_manager, first_state, rep);
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ROS_WARN("[GlobalGraph] Added successfully.");
...
bool Rrg::connectStateToGraph(std::shared_ptr<GraphManager> graph,
  ...
  if (direction_norm <= kDelta) {
    ...
    ROS_WARN("[GlobalGraph] Try to add current state to the graph.");
    ExpandGraphReport rep;
    expandTreeStar(graph, cur_state, rep);
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ROS_WARN("[GlobalGraph] Added successfully.");
