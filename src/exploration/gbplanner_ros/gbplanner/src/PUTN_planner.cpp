#include "gbplanner/PUTN_planner.h"
...
void PFRRTStar::initWithoutGoal(const Vector3d &start_pos)
{
    ...
    Node* node_origin=fitPlane(start_pos);//以此点拟合固定半径的平面
    ...
    if(last_planning_state!=WithoutGoal || !inheritTree(node_origin))//剪枝
    {
        clean_vector(tree_);
        node_origin_=node_origin;
        tree_.push_back(node_origin_);
...
void PFRRTStar::trimTree()
{
    ...
    addInvalidNodes(node_origin_,false,invalid_nodes);//添加无效点
    for(auto &node:invalid_nodes)
    {
        if(node->parent_!=NULL) deleteChildren(node->parent_,node);//删除子节点
...
bool PFRRTStar::planner(float &terrain_cost, Eigen::Vector3d &sample_point)
{
    // if(planning_state_==Invalid)
    // {
    //     ROS_ERROR("Illegal operation:the planner is at an invalid working state!!");
    ...
    // {
        ...
        // //Find the nearest node to the random point
        // Node* nearest_node= findNearest(rand_point_2D);
        Vector2d nearest_point_2D = project2plane(sample_point);

        //Expand from the nearest point to the random point
        // Vector2d new_point_2D = steer(rand_point_2D,nearest_point_2D);

        //Based on the new 2D point,
        Node* new_node = fitPlane(nearest_point_2D);
        ...
        {   
            terrain_cost = new_node->plane_->traversability;
            sample_point = new_node->position_;

//             if(terrain_cost<1.0&&terrain_cost>=0.9)
//             {   
...
//                   float  flatness=new_node->plane_->flatness_;
//                     float slope=new_node->plane_->slope_;
//                    float sparsity=new_node->plane_->sparsity_;
//                  std::cout<<"**********: "<<std::endl;
//                 std::cout<<"flatness: "<<flatness<<", flatness_w: "<<4000.0*flatness<<std::endl;
//                 std::cout<<"slope: "<<slope<<", slope_w: "<<0.4*slope<<std::endl;
//                 std::cout<<"sparsity: "<<sparsity<<", sparsity_w: "<<0.4*sparsity<<std::endl;
               
//                 std::cout<<"total: "<<4000.0*flatness+0.4*slope +0.4*sparsity<<std::endl;
