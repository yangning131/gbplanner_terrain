...
class Rrg {
 ...
 bool circle_collision(double x1, double y1,double x2,double y2);
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr surroundMapCloud;
  pcl::PointCloud<pcl::PointXYZ>::Ptr surroundMapCloudDS;
 ...
 private:
  ...
  // Randomly sample a vertex from a predefined PDF, used for the local planner.
  bool sampleVertex(StateVec& state,  vec_E<Polyhedron2D>& polys);  
...
///--------------------
  ...
  ros::Subscriber cloud_terrain_sub;
