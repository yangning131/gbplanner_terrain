...
class MapManagerVoxblox : MapManager {
 public:
  ...
  void getsdfsurface(pcl::PointCloud<pcl::PointXYZI>::Ptr pointcloud)
  {
      //  std::cout<<"pointcloud "<<pointcloud<<std::endl;
        pcl::PointCloud<pcl::PointXYZI> point = *pointcloud;
        const double surface_distance_thresh_m =
        sdf_layer_->voxel_size() * 2.0;
        voxblox::createSurfaceDistancePointcloudFromTsdfLayer(
        *sdf_layer_, surface_distance_thresh_m, &point);
