...
void TrajectoryNLP::BuildIterativeNLP() {
  ...
  auto g_x_kin = x(next) - (x(prev) + hi * v(prev) * cos(theta(prev))*cos(p_fit_alpha(prev)));//cos(a)
  auto g_y_kin = y(next) - (y(prev) + hi * v(prev) * sin(theta(prev))*cos(p_fit_alpha(prev)));
  auto g_theta_kin = theta(next) - (theta(prev)+atan(1/cos(p_fit_alpha(prev))*tan(omega(prev)*hi)));
  ...
  auto infeasibility = sumsqr(SX::vertcat({
                                            g_x_kin, g_y_kin, g_theta_kin, g_v_kin, g_a_kin, g_xf_kin,
                                            g_yf_kin, g_xr_kin, g_yr_kin}));
  ...
  SX opti_xand = SX::vertcat({x, y, theta, v, a, omega, jerk, xf, yf, xr, yr, p_fit_alpha});
...
double TrajectoryNLP::SolveIteratively(double w_inf, const Constraints &constraints, const States &guess,
  ...
  DM lb_x, lb_y, lb_theta, lb_v, lb_a, lb_omega, lb_jerk, lb_xf, lb_yf, lb_xr, lb_yr;
  DM ub_x, ub_y, ub_theta, ub_v, ub_a, ub_omega, ub_jerk, ub_xf, ub_yf, ub_xr, ub_yr;
  ...
  DMDict arg, res;
  arg["lbx"] = DM::vertcat({lb_x, lb_y, lb_theta, lb_v, lb_a, lb_omega, lb_jerk, lb_xf, lb_yf, lb_xr, lb_yr});
  arg["ubx"] = DM::vertcat({ub_x, ub_y, ub_theta, ub_v, ub_a, ub_omega, ub_jerk, ub_xf, ub_yf, ub_xr, ub_yr});
  arg["x0"] = DM::vertcat(
    {guess.x, guess.y, guess.theta, guess.v, guess.a, guess.omega, guess.jerk, guess.xf, guess.yf, guess.xr,
     guess.yr});
  ...
  for (size_t i = 0; i < config_.nfe; i++) {
    ...
    result.theta[i] = double(opt(2 * config_.nfe + i, 0));
    result.v[i] = double(opt(3 * config_.nfe + i, 0));
    result.a[i] = double(opt(4 * config_.nfe + i, 0));
    result.omega[i] = double(opt(5 * config_.nfe + i, 0));
    result.jerk[i] = double(opt(6 * config_.nfe + i, 0));
    result.xf[i] = double(opt(7 * config_.nfe + i, 0));
    result.yf[i] = double(opt(8 * config_.nfe + i, 0));
    result.xr[i] = double(opt(9 * config_.nfe + i, 0));
    result.yr[i] = double(opt(10 * config_.nfe + i, 0));
  }
