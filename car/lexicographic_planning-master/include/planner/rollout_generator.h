 ...
 * @return: 
 */
//openPlannerRollOut.run(transform,                                                 globalPathMessage喂入路径 ,                    globalPath,                           searchedPath,                                 remainingPath, alternativePaths);
void RolloutGenerator::run(tf::StampedTransform transform, nav_msgs::Path pathMsg, nav_msgs::Path& fixedGlobal, nav_msgs::Path& centerPath, nav_msgs::Path& remainingPath, vector<nav_msgs::Path>& alternativePaths)
{
    ...
    getGlobalPlannerPath_cb(pathMsg);//接收的路径转换为waypoint放入globalPaths（多组waypoint）数组里，每个路径间隔点的方向，及一条路径上waypoint的累计代价（距离代价）

    if (globalPaths.size() == 0)
        return;

    globalPathSections.clear();//多组waypoint路径组
    for (size_t i = 0; i < globalPaths.size(); i++) {
        centralTrajectorySmoothed.clear();//中心路径组
        extractPartFromTrajectory(globalPaths[i], current_pose, PlanningParams.microPlanDistance,   //从全局路径截取最大局部规划距离     固定路径密度，不超过pathDensity + margin   代价  结果centralTrajectorySmoothed
            PlanningParams.pathDensity, centralTrajectorySmoothed);
        globalPathSections.push_back(centralTrajectorySmoothed);//结果globalPathSections
    }  
    ...
    centerPath       = getCenterPath(centralTrajectorySmoothed);//主要是yaw转四元数
    remainingPath    = getRemainingPath(fixedGlobal, centerPath);//////？？？？？
    alternativePaths = getOtherpaths(rollOuts[0]);//主要是yaw转四元数
...
nav_msgs::Path RolloutGenerator::getRemainingPath(nav_msgs::Path pathIn, nav_msgs::Path centerPath)//(fixedGlobal, centerPath) 路径最后一段
{
    nav_msgs::Path pathOut = pathIn;

    geometry_msgs::Point lastPose = centerPath.poses.back().pose.position;//中心路径的最后一点
...
/**
 * @description: 从全局路径截取最大局部规划距离     固定路径密度，不超过pathDensity + margin   代价
 * @param {type} 
 * @return: 
 */
    ...
    vector<UtilityNS::WayPoint>& extractedPath)
{
    if (originalPath.size() < 2)
        return;
    extractedPath.clear();
    int close_index = UtilityNS::getNextClosePointIndex(originalPath, currentPos);//获取轨迹上距离当前位置最近的轨迹点（前方）
    // printf("%d\n", close_index);
    ...
    for (int i = close_index; i >= 0; i--) {
        ...
        if (i < originalPath.size())
            dis += hypot(originalPath[i].pos.y - originalPath[i + 1].pos.y, originalPath[i].pos.x - originalPath[i + 1].pos.x);//返回斜边
        // printf("%f\n", dis);
        if (dis > 2)
            break;
    }
    ...
    fixPathDensity(extractedPath, waypointDensity);//固定路径密度，不超过pathDensity + margin
    calcAngleAndCost(extractedPath);
}

void RolloutGenerator::fixPathDensity(vector<UtilityNS::WayPoint>& path, const double& pathDensity)//固定路径密度，不超过pathDensity + margin
{
...
void RolloutGenerator::getGlobalPlannerPath_cb(nav_msgs::Path msg)//接收的路径转换为waypoint放入globalPaths数组里，每个路径间隔点的方向，及一条路径上waypoint的累计代价（距离代价）
{
    if (msg.poses.size() > 0) {
        globalPaths.clear();
        vector<UtilityNS::WayPoint> single_path;
        msgLane2LocalLane(msg, single_path);//把路径点放数组里
        calcAngleAndCost(single_path);//每个路径间隔点的方向，及一条路径上waypoint的累计代价（距离代价）
...
double RolloutGenerator::calcAngleAndCost(vector<UtilityNS::WayPoint>& path)
{
    if (path.size() < 2)
        return 0;
    if (path.size() == 2) {
        path[0].pos.yaw = UtilityNS::cast_from_0_to_2PI_Angle(atan2(path[1].pos.y - path[0].pos.y, path[1].pos.x - path[0].pos.x));//求两点间的角度，将当前角度转换到0～2pi 
        path[0].cost = 0;
        path[1].pos.yaw = path[0].pos.yaw;
        path[1].cost = path[0].cost + distance2points(path[0].pos, path[1].pos);//两点的距离代价
        return path[1].cost;
    }
    ...
    for (int j = 1; j < path.size() - 1; j++) {
        ...
        path[j].cost = path[j - 1].cost + distance2points(path[j - 1].pos, path[j].pos);//一条路径上waypoint的累计代价（距离代价）
    }

    int j = (int)path.size() - 1;
    path[j].pos.yaw = path[j - 1].pos.yaw;//最后一个点的角度
