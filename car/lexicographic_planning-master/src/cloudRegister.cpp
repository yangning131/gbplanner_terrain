...
public:
    ...
    pcl::VoxelGrid<PointType> downSizeFilter;//体素滤波下采样

    CloudRegister()
    {   
        subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>(_pointCloudTopic, 1, &CloudRegister::cloudHandler, this);
        pubLaserCloud = nh.advertise<sensor_msgs::PointCloud2>("planning/registered_cloud_all", 1);
        
        newCloudFlag = false;
        laserCloudIn.reset(new pcl::PointCloud<PointType>());

        downSizeFilter.setLeafSize(_mapResolution, _mapResolution, _mapResolution);//体素网格大小
        // downSizeFilter.setLeafSize(0.01, 0.01, 0.01);//体素网格大小

    }

    void cloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {   
        if (pubLaserCloud.getNumSubscribers() == 0)//根据是否有节点订阅所需话题，决定是否发布对应话题
            return;

        try{
            listener.waitForTransform("map", "base", laserCloudMsg->header.stamp, ros::Duration(1.0));
            listener.lookupTransform("map", "base", laserCloudMsg->header.stamp, transform);
        } 
    ...
    void run()
    {
        ...
        for (int i = 0; i < laserCloudInGlobal->size(); ++i)
        {
            ...
            // float range = pointDistance(p, robotPoint);//点云和机器人的距离
            // if (range < _sensorRangeLimitMin || range > _sensorRangeLimitMax)
            //     continue;

            laserCloudFiltered->push_back(p);//距离在规定的范围内放入 发布
        }
