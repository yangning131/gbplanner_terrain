...
public:
    ...
    void cloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        PointType robotPoint;//机器人坐标    pcl::PointXYZI 
        ...
        // update occupancy map  更新障碍物地图
        if (pubOccupancyMap2.getNumSubscribers() != 0)//接收到话题
        {
            occupancyMap2D.header.stamp = laserCloudMsg->header.stamp;
            occupancyMap2D.info.origin.position.x = robotPoint.x - _local_map_length / 2.0;//左下角相对于map的坐标
            ...
            int search_num_ob = round(_occuMapInflation / _mapResolution);//膨胀面积/山歌分辨率=膨胀面积内的山歌个数
            for (int i = 0; i < surroundMapCloudDS->size(); ++i)
            {
                int index_x = (surroundMapCloudDS->points[i].x - occupancyMap2D.info.origin.position.x) / _mapResolution;//点云在局部障碍地图的索引
                ...
                for (int m = -search_num_ob; m <= search_num_ob; ++m)
                {
                    ...
                    {
                        if (sqrt(float(m*m + n*n)) * _mapResolution > _occuMapInflation)//大于膨胀面积跳出
                            continue;

                        int x_id = index_x + m;
                        int y_id = index_y + n;

                        if (x_id < 0 || y_id < 0 || x_id >= _local_map_grid_num || y_id >= _local_map_grid_num)//超出范围不计
                            continue;

                        int index = y_id * occupancyMap2D.info.width + x_id;//珊格地图索引
                        occupancyMap2D.data[index] = 100;//有障碍物赋值
                    }
                }
            }
            ...
            int search_num_field = round(_occuMapField / _mapResolution);//再膨胀个数
            ...
            for (int i = 0; i < occupancyMap2D.info.width; ++i)
            {
                ...
                {
                    if (occupancyMap2D.data[j * occupancyMap2D.info.width + i] == 100)//有障碍物赋过值
                    {
