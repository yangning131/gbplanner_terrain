...
public:
    ...
    ros::Publisher pubPRMGraph;//概率路线图
    ...
    vector<state_t*> nodeList;//顶点位置、邻边代价、父节点代价
    vector<vector<state_t*> > adjacencyMatrix;  //邻接矩阵
    ...
    PathPlanning()
    {
        ...
        pathUpdateTimer = nh.createTimer(ros::Duration(1.0/10.0), &PathPlanning::updatePath, this);//定时每0.1s调用updatePath回调
    }
    ...
    void pathHandler(const nav_msgs::Path::ConstPtr& pathMsg)
    {
     std::lock_guard<std::mutex> lock(mtx);
        ...
        // treate the original centerPath from open_planner as searched path  将openplaner的中线路径作为searched path
        openPlannerRollOut.run(transform, globalPathMessage, globalPath, searchedPath, remainingPath, alternativePaths);
        executePath = combinePaths(searchedPath, remainingPath);//centerPath搜索的中心路径   remainingPath路径最后一段
    ...
    void buildAdjacencyMatrix()
    {
        ...
        for (int i = 0; i < alternativePaths.size(); ++i)   //将路径上的点放入矩阵
        {
    ...
    void connectAdjacencyMatrix()
    {
        ...
        for (int i = 0; i < nodeList.size(); ++i)
        {
            ...
            {
                nodeList[i]->validFlag = false;//有效标志
                continue;
            }
        }
        ...
        for (int i = 0; i < adjacency_width_grid; ++i)
        {
            ...
            {
                if (adjacencyMatrix[i][j]->validFlag == false)//只作用有效果点
                    continue;
    ...
    bool edgePropagation(state_t *state_from, state_t *state_to, float edgeCosts[NUM_COSTS]){
        ...
        for (int stepCount = 0; stepCount < steps; ++stepCount)
        {
            ...
            // close to obstacle cost  障碍物接近成本
        ...
        while(Queue.size() > 0 && ros::ok())
        {
            ...
            for (int i = 0; i < fromState->neighborList.size(); ++i)
            {
                ...
                for (int costIndex = 0; costIndex < NUM_COSTS; ++costIndex)
                {
                    ...
                    // If cost is same, go to compare secondary cost  按顺序循环优化，以上层代价为主 上层代价最小的情况下，就不考虑其他后面的代价了！！！
    ...
    bool needNewPath()
    {
        ...
        for (int i = 0; i < executePath.poses.size(); ++i)
        {
            ...
            if (isIncollision(x, y))// || isCloseCollision(x, y))//判断碰撞
            {
